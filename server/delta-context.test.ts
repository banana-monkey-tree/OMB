...
describe("Handoffs", () => {
  const setup = (order: string[], record?: HandedState) => {
    ...
    const start = (handoff: Partial<Handoff>) => {
      handoffs.begin("t", "claim", {
        botId: "b", instanceId: "claude", config: "c", started: { sent: [] }, rebuilt: { sent: [] }, resumed: { sent: [] },
  ...
  it("clears the record when another session carries the turn, and credits that session only with its rebuild", () => {
    ...
    f.start({ resumeCursor: "old", placed: ["m8"], carried: ["m9"], rebuilt: sessionStart(order, ["m5", "m6", "m7"], ["m9"]) });
  ...
  it("leaves a replacement that is never accepted without a record", () => {
    ...
    f.start({ resumeCursor: "old", carried: ["m3"], own: ["m3"], rebuilt: { sent: ["m0", "m1", "m2", "m3"] } });
    f.event({ type: "session.started", sessionId: "new" });
  ...
  it("credits a steer only after output that follows it, and withdraws it when the person stops the turn", () => {
    const order = ids(6);
    const f = setup(order, { session: "s", through: "m2", ids: [] });
    f.start({ resumeCursor: "s", carried: ["m3"], own: ["m3"] });
    f.event(output);
    const target = f.handoffs.current("t");
    f.handoffs.steered("t", target, "claude", "m4");
    f.event({ type: "turn.completed", ok: false });
